...
def find_station(query):
    """
    Find station using fuzzy matching.
    Returns (matched_station, confidence) or (None, 0) if no match found.
    """
    if query in KG:
        return query, 1.0  # Exact match

    # Fuzzy search with cutoff=0.4 for lenient matching
    matches = get_close_matches(query, KG.nodes(), n=1, cutoff=0.4)
    if matches:
        return matches[0], 0.8  # Fuzzy match
    return None, 0
